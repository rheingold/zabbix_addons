 ...
 * 
 * PURPOSE:
 * Widget view template - renders graph display in dashboard.
 * Handles error states, no-data states, and successful graph rendering.
 * Creates HTML5 Canvas elements and passes data to JavaScript.
 * 
 * RELATIONS:
 * - Called by: actions/WidgetView.php doAction() method
 * - Receives data from: actions/WidgetView.php via $data array
 * - Renders for: assets/js/class.widget.js (client-side rendering)
 * - Extends: CWidgetView (Zabbix widget view framework)
 * 
 * DATA STRUCTURE:
 * $data = [
 ...
 *   'graph_data' => [
 *     'series' => [[name, data, color, units], ...],
 *     'legend_position' => int,
 *     'fill_opacity' => float,
 *     ... (see GraphData.php for full structure)
 *   ]
 * ]
 * 
 * RENDERING MODES:
 * 1. Error mode: Display error message
 * 2. No data mode: Display "No graph data available"
 * 3. Graph mode: Render canvas elements with graph_data
 * 
...
// === NO DATA STATE: Display message when no series available ===
...
// === GRAPH RENDERING: Create canvas elements for graph display ===
// Generate unique ID for this graph instance (prevents conflicts with multiple widgets)
